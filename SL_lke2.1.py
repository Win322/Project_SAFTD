...
class MainWindow(QtWidgets.QMainWindow, Ui_SAFTD):
    def __init__(self):
        ...
        self.tableWidget_3.cellChanged.connect(self.cell_changed_tb3)
        ...
        self.tableWidget_3.cellChanged.connect(self.cell_changed_tb3_2)
    ...
    def cell_changed_tb3(self, row, column):
        if column == 4:
            item = self.tableWidget_3.item(row, 4)
            item = item.text() if (item and item.text()) else '0'
            _delta = str(int(item) // self.delta)

            item = QTableWidgetItem(_delta)
            item.setFlags(QtCore.Qt.ItemIsEnabled)
            self.tableWidget_3.setItem(row, 5, item)

    def cell_changed_tb3_2(self, row, column):
        if column == 6:
            item = self.tableWidget_3.item(row, 6)
            item = item.text() if (item and item.text()) else '0'
            _delta = str(int(item) // self.delta)

            item = QTableWidgetItem(_delta)
            item.setFlags(QtCore.Qt.ItemIsEnabled)
            self.tableWidget_3.setItem(row, 7, item)
    ...
    def buttonSaveProject(self):
        ...
        if len(directory_file) > 0 and len(name_folder) > 0:
            if os.path.exists(directory_file) == True:
                ...
                msg.setText("Данный путь: " + directory_file + " не существует")
    ...
    def buttonAddtable3(self):
        ...
        item = QTableWidgetItem()
        item.setFlags(QtCore.Qt.ItemIsEnabled)
        self.tableWidget_3.setItem(rowPosition, 5, item)

        item = QTableWidgetItem()
        item.setFlags(QtCore.Qt.ItemIsEnabled)
        self.tableWidget_3.setItem(rowPosition, 7, item)
